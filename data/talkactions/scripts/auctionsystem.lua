...
function onSay(cid, words, param, channel)
        ...
        if(t[1] == "add") then
                ...
                if(not tonumber(t[3]) or (not tonumber(t[4]))) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce nao definiu um preco valido ou a quantidade de itens.")
                        return true
                end
                if(string.len(t[3]) > 7 or (string.len(t[4]) > 3)) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Este preco ou quantidade de itens e muito alto.")
                        return true
                end
                local item = getItemIdByName(t[2])
                if(not item) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Este item nao existe.")
                        return true
                end
                if(getPlayerLevel(cid) < config.levelRequiredToAdd) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce precisa ser level " .. config.levelRequiredToAdd .. " para usar esse sitema.")
                        return true
                end
                if(isInArray(config.blocked_items, item)) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Este item esta bloqueado.")
                        return true
                end
                if(getPlayerItemCount(cid, item) < (tonumber(t[4]))) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Desculpe, voce nao tem este(s) item(ns).")
                        return true
                end
                ...
                elseif(check:getRows(true) >= config.maxOffersPerPlayer) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Desculpe, voce nao pode adicionar mais ofertas, max: " .. config.maxOffersPerPlayer .. ".")
                        return true
                end
                if(config.SendOffersOnlyInPZ) then    
                        if(not getTilePzInfo(getPlayerPosition(cid))) then
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce deve estar na area PZ ao adicionar uma oferta.")
                                return true
                        end
                end
                if(tonumber(t[4]) < 1 or (tonumber(t[3]) < 1)) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce tem que digitar um numero maior que 0.")
                        return true
                end
                ...
                db.executeQuery("INSERT INTO `auction_system` (`player`, `item_name`, `item_id`, `count`, `cost`, `date`) VALUES (" .. getPlayerGUID(cid) .. ", \"" .. t[2] .. "\", " .. getItemIdByName(t[2]) .. ", " .. itemcount .. ", " .. costgp ..", " .. os.time() .. ")")
                                doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Voce adicionou " .. itemcount .." " .. t[2] .." por " .. costgp .. " gps.")
        end
        if(t[1] == "buy") then
                ...
                if(buy:getID() ~= -1) then
                        if(getPlayerMoney(cid) < buy:getDataInt("cost")) then
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce nao tem dinheiro suficiente.")
                                buy:free()
                                return true
                        end
                        if(getPlayerName(cid) == getPlayerNameByGUID(buy:getDataInt("player"))) then
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Desculpe, voce nao pode comprar seu proprios item. Para remover um item digite: !shop remove, NOME")
                                buy:free()
                                return true
                        end
                        if(getPlayerFreeCap(cid) < getItemWeightById(buy:getDataInt("item_id"), buy:getDataInt("count")))then
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce tentou comprar um " .. buy:getDataString("item_name") .. ". Com um peso de " .. getItemWeightById(buy:getDataInt("item_id"), buy:getDataInt("count")) .. " Cap oz, e voce possue " .. getPlayerFreeCap(cid) .. " Cap oz. Esvazia sua backpack e tente novamente.")
                                buy:free()
                                return true
                        end
                        ...
                        doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Voce comprou " .. buy:getDataInt("count") .. " ".. buy:getDataString("item_name") .. " por " .. buy:getDataInt("cost") .. " gps!")
        ...
        if(t[1] == "remove") then
                if((not tonumber(t[2]))) then
                        ...
                        return true
                                ...
                                if(config.SendOffersOnlyInPZ) then    
                                        if(not getTilePzInfo(getPlayerPosition(cid))) then
                                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce deve estar na area PZ ao remover ofertas do Shop.")
                                                return true
                                        end
                end
                ...
                if(delete:getID() ~= -1) then
                        if(getPlayerGUID(cid) == delete:getDataInt("player")) then
                                ...
                                doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Sua oferta foi excluida do Shop.")
                        else
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Este item nao te pertence!")
                        end
                delete:free()
                else
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "ID invalido.")
        ...
        if(t[1] == "withdraw") then
                ...
                if(balance:getDataInt("auction_balance") < 1) then
                        doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Voce nao possue saldo no Shop.")
                        balance:free()
                        return true
                end
                doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Voce sacou " .. balance:getDataInt("auction_balance") .. " gps do Shop!")
