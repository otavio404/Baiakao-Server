...
function MiniGame:End(cooldown,game)
	addEvent(function()
		...
		self:deleteBalista()
		self:deleteBird()
		local exp, gold = math.ceil(((getPlayerLevel(self.cid) / 100) + 1.5)) * self:getPoints() * 4000, self:getPoints() * 3000
		doPlayerSendTextMessage(self.cid, MESSAGE_STATUS_CONSOLE_BLUE, "Você acertou o pássaro "..self:getPoints().." vezes.")
