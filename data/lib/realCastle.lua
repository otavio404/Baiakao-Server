...
realCastle.close = function()
	...
	if getGlobalStorageValue(realCastle.guild_storage) == -1 then
		doBroadcastMessage("[Insanity Castle] A batalha terminou e nenhuma guild conseguiu dominar o Castelo! Amanhã às 21:00 vocês podem tentar novamente!")
	else
		doBroadcastMessage("[Insanity Castle] A batalha terminou e a guild vencedora foi ["..getGuildNameByID(getGlobalStorageValue(realCastle.guild_storage)).."]! Todos os jogadores dessa guild agora podem aproveitar o Castelo até amanhâ às 21:00!")
	end
	doSaveServer()
end
...
realCastle.domain = function(cid)
	...
	realCastle.broadcast("[Insanity Castle] O jogador "..getPlayerName(cid).." da guild ["..getPlayerGuildName(cid).."] dominou o Castelo! As outras guilds têm até às 22:20 para conseguir tirar o domínio deles, não desistam!")
