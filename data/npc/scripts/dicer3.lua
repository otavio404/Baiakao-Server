...
local function rollDice(roll, cc_count, pc_count, table_left_pos, table_middle_pos, npc)
	...
	if ((total_g) <= 1000000 and (total_g) >= 5000) then
		...
		if (roll == 1 and random_rollval <= 3) then
			...
			addEvent(doCreatureSay, 500, npc, "Voce venceu e ganhou "..total_g.." gold coins, Parabens!", TALKTYPE_SAY, false, 0)
		elseif (roll == 2 and random_rollval >= 4) then
			...
			addEvent(doCreatureSay, 500, npc, "Voce perdeu tente denovo.", TALKTYPE_SAY, false, 0)
		end
		doCreatureSay(npc, string.format("%s sorteou o numero: %d.", getCreatureName(npc), random_rollval), TALKTYPE_ORANGE_1, false, 0, table_left_pos)
	else
		addEvent(doCreateItem, 100, 2160, cc_count, table_middle_pos)
		addEvent(doCreateItem, 150, 2152, pc_count, table_middle_pos)
		doCreatureSay(npc, "Voce precisa jogar o dinheiro no meio do depot e falar L para 1 a 3 ou H para 4 a 6 o preco minimo para a aposta e 5k e o maximo 1kk.", TALKTYPE_SAY, false, 0)
	end
	return true
end
...
function creatureSayCallback(cid, type, msg)
	...
	local table_left_pos = {x = 524, y = 88, z = 14}
	local table_middle_pos = {x = 525, y = 88, z = 14}
