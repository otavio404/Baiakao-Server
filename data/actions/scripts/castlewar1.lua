...
local Castle24Hrs_one_name = "Castle24Hrs"                  -- Nome do castelo 1
...
local teleport = {x=1349, y=1504, z=7}
   ...
   if isPlayerGuild(cid) == TRUE then
      if sto_ativ == 1 or sto_ativ == -1 then
         ...
         guild_sto = getGlobalStorageValue(storages[3])
            ...
            if guild ~= guild_sto then            
               ...
               doBroadcastMessage("[Castle24Hrs] O(a) "..getCreatureName(cid).." acabou de conquistar o castelo "..Castle24Hrs_one_name.." para a guild \""..guildname.."\" pela "..sto_vezes_total.."ª vez(s).", 22)
                 ...
                 if premio_por_vezes == 1 then
                    if getPlayerStorageValue(cid,storages[1]) == premio_vezes then
                       if getItemWeightById(premio,1) <= getPlayerFreeCap(cid) then
                          doBroadcastMessage("[Castle24Hrs] O "..getCreatureName(cid).." ganhou uma(o) "..getItemNameById(premio).." como recompensa das suas "..premio_vezes.." conquistas!", 21)
      ...
      doPlayerSendCancel(cid,"[Castle24Hrs] O sistema está desativado")
      end      
   else
   doPlayerSendCancel(cid,"[Castle24Hrs] É nescessario ter guild para dominar o castelo!")
   doSendMagicEffect(position, 2)
   end
end
...
function additem(cid,premio,premio_cont,premio_reset)
 ...
 if getItemWeightById(premio,1) <= getPlayerFreeCap(cid) then
  ...
  doPlayerSendTextMessage(cid, 18, "[Castle24Hrs24Hrs] Voce nao tem capacidade para ganhar o item. Verificando novamente em 10 segundos!")
